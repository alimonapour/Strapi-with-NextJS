 ...
 */ const build = async ({ logger, cwd, tsconfig, ...options })=>{
    ...
    try {
        ...
        if (ctx.bundler === 'webpack') {
            const { build: buildWebpack } = await import('./build-gD9rqI7P.mjs');
            await buildWebpack(ctx);
        } else if (ctx.bundler === 'vite') {
            const { build: buildVite } = await import('./build-DX22_Xk-.mjs');
            await buildVite(ctx);
        }
...
const develop = async ({ cwd, polling, logger, tsconfig, watchAdmin, ...options })=>{
     ...
     */ let bundleWatcher;
        ...
        if (watchAdmin) {
            ...
            await writeStaticClientFiles(ctx);
            if (ctx.bundler === 'webpack') {
                const { watch: watchWebpack } = await import('./watch-GFfg5oE3.mjs');
                bundleWatcher = await watchWebpack(ctx);
            } else if (ctx.bundler === 'vite') {
                const { watch: watchVite } = await import('./watch-CNSuaMw6.mjs');
...
//# sourceMappingURL=index--KO67yy1.mjs.map
