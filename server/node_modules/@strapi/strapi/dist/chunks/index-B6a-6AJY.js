 ...
 */ const build = async ({ logger, cwd, tsconfig, ...options })=>{
    ...
    try {
        ...
        if (ctx.bundler === 'webpack') {
            const { build: buildWebpack } = await Promise.resolve().then(function () { return require('./build-CtJmeNVA.js'); });
            await buildWebpack(ctx);
        } else if (ctx.bundler === 'vite') {
            const { build: buildVite } = await Promise.resolve().then(function () { return require('./build-BdILbhJl.js'); });
            await buildVite(ctx);
        }
...
const develop = async ({ cwd, polling, logger, tsconfig, watchAdmin, ...options })=>{
     ...
     */ let bundleWatcher;
        ...
        if (watchAdmin) {
            ...
            await writeStaticClientFiles(ctx);
            if (ctx.bundler === 'webpack') {
                const { watch: watchWebpack } = await Promise.resolve().then(function () { return require('./watch-DTWWenI8.js'); });
                bundleWatcher = await watchWebpack(ctx);
            } else if (ctx.bundler === 'vite') {
                const { watch: watchVite } = await Promise.resolve().then(function () { return require('./watch-5yscCmUn.js'); });
...
//# sourceMappingURL=index-B6a-6AJY.js.map
